...
xlua.private_accessible(CS.TargetTrainRankItem)

local CheckTeamDetail = function(self)
	local data = CS.TargetTrainRecordData()
	if self.rankData.targetTrainLog ~= nil and self.rankData.targetTrainLog ~= '' then
		local jsonData = CS.LitJson.JsonMapper.ToObject(self.rankData.targetTrainLog)
	...
	settlement:InitByRecord(data, self.rankData.name)
end
util.hotfix_ex(CS.TargetTrainRankItem,'CheckTeamDetail',CheckTeamDetail)